...
`Backend/docs/CURRENT_RELEASE_STATUS.md` is the authoritative cross-repository status record. The separate frontend/backend Deep Security Scans sealed on 2026-08-28 are historical evidence for their exact source pair; authentication, recruitment, communications, migration, and UI code changed afterward. The current source pair therefore requires fresh affected security and release qualification. Staging browser evidence, institutional privacy approval, representative user acceptance, immutable artifact identity, and final authorization remain release gates.
