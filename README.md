...
| --- | --- | --- |
| Generative AI | A model produces new wording or structured content | The bounded preparation pilot uses configured Gemini structured generation; resume extraction and conservative wording suggestions remain rule-based |
...
| Agentic orchestration | Coordinate stateful steps with explicit boundaries | Student opportunity preparation and T&P drive review use finite plan → clarify or draft workflows with budgets, evidence validation, interruption/resume, and human review; the policy flow remains fixed retrieve → explain |
...
### Additional generative AI extension points
...
These are **extension ideas beyond the bounded preparation pilot, not shipped capabilities**. See the [backend AI walkthrough](https://github.com/painjanevivek/CampusHire-backend#agentic-ai-and-generative-ai) for the actual graphs, provider adapter, and document pipeline.
