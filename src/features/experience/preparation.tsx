...
function RolePreparation({ roleId }: { roleId: string }) {
  ...
  return <section className={styles.stack} aria-label="Opportunity preparation" aria-busy={loading}>
    ...
    {loading && <p role="status">Checking your profile details…</p>}
    ...
    {data && <>
      <section className={styles.panel} data-primary="true"><h2>Prepare for {data.role_title}</h2><p>This is preparation guidance, not an eligibility decision. No AI generation runs when you open this page.</p>
        ...
        <details><summary>Source resume</summary><p className={styles.muted}>Resume: {data.source_resume_version_id ?? "Not available"}</p></details>
      </section>
      <section className={styles.panel}><h2>Requirements and your profile</h2><ul>{data.evidence.map(item => <li key={item.requirement}><h3>{item.requirement}</h3><p>{item.evidence}</p></li>)}</ul>
