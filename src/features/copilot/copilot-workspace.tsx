...
export function CopilotWorkspace({ audience }: { audience: Audience }) {
  ...
  return (
    <main id="main-content" className={styles.page}>
      <header><div><p>{audience === "student" ? "Student guidance" : "T&P proposal workspace"}</p><h1>{audience === "student" ? "Student Copilot" : "T&P Copilot"}</h1><span>{audience === "student" ? "Grounded answers with deterministic eligibility evidence." : "Draft and analyze. Approval never publishes or sends."}</span></div>{conversation ? <div>{conversations.length ? <label>Conversation<select value={conversation.id} onChange={(event) => { setConversation(conversations.find((item) => item.id === event.target.value) ?? null); setProposal(null); }}>{conversations.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label> : null}<button type="button" onClick={() => void start()} disabled={busy}>New conversation</button><button type="button" onClick={() => void remove()}><Trash2 aria-hidden="true" /> Delete conversation</button></div> : null}</header>
