...
type RoleOption = { id: string; title: string; company_name: string };
type OpportunityPage = { items: RoleOption[] };
...
const studentIntents = [
  ...
  ["interview_practice", "Practise an interview answer"],
...
export function CopilotWorkspace({ audience }: { audience: Audience }) {
  ...
  useEffect(() => {
    ...
    void apiRequest<OpportunityPage>("/opportunities", { cache: "no-store" })
      .then((page) => setRoleOptions(page.items))
