...
export function AiResumeStudio({ initialProposalId }: { initialProposalId?: string }) {
  ...
  return (
    <main id="main-content" className={styles.page}>
      <header><p>Generative AI · proposal only</p><h1>AI Resume Studio</h1><span>Choose profile details → draft with AI → check each statement → edit → approve → create your PDF.</span></header>
      {message ? <Alert tone={state === "error" ? "error" : "success"}>{message}</Alert> : null}
      {!proposal ? <section className={styles.panel}><h2>Choose profile details</h2><p>Direct identifiers are excluded from the AI request. Each generated statement shows which profile details it came from.</p><div className={styles.evidence}>{evidence.map((item) => <label key={item.evidence_id}><input type="checkbox" checked={selected.includes(item.evidence_id)} onChange={(event) => setSelected((current) => event.target.checked ? [...current, item.evidence_id] : current.filter((id) => id !== item.evidence_id))} /><span><strong>{item.label}</strong><small>{item.kind}</small></span></label>)}</div>{state === "loading" ? <LoaderCircle aria-hidden="true" /> : null}<button type="button" onClick={() => void generate()} disabled={state === "busy" || selected.length === 0}><Sparkles aria-hidden="true" /> {state === "busy" ? "Generating grounded draft…" : "Generate proposal"}</button><Link href="/resume/builder">Use manual builder instead</Link></section> : null}
      {proposal && draft ? <section className={styles.panel}><div className={styles.provenance}><Sparkles aria-hidden="true" /><span><strong>Generated proposal</strong><small>{proposal.provider_name} · {proposal.model_version} · {proposal.prompt_version}</small></span></div><div className={styles.claims}>{draft.professional_summary ? <label><span>Professional summary</span><textarea value={draft.professional_summary.text} onChange={(event) => editClaim("professional_summary", 0, event.target.value)} rows={4} /><small>Based on: {draft.professional_summary.evidence_ids.map((id) => evidenceMap.get(id)?.label ?? id).join(", ")}</small></label> : null}{(["education", "project_bullets", "experience_bullets", "skills"] as const).flatMap((section) => draft[section].map((claim, index) => <label key={`${section}-${index}`}><span>{section.replaceAll("_", " ")} {index + 1}</span><textarea value={claim.text} onChange={(event) => editClaim(section, index, event.target.value)} rows={3} /><small>Based on: {claim.evidence_ids.map((id) => evidenceMap.get(id)?.label ?? id).join(", ")}</small></label>))}</div><Alert tone="info"><Check aria-hidden="true" /> Accepting creates a new version. It never edits your profile or overwrites an earlier resume.</Alert>{proposal.status === "draft" ? <div className={styles.actions}><button type="button" className={styles.reject} onClick={() => void decide("reject")} disabled={state === "busy"}>Reject</button><button type="button" onClick={() => void decide("accept")} disabled={state === "busy"}><FileText aria-hidden="true" /> Accept and create PDF</button></div> : <Link href="/resume">Compare and download versions</Link>}</section> : null}
    </main>
  );
}
