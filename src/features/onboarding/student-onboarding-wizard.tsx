...
import { ArrowLeft, ArrowRight, Check, Cloud, LoaderCircle, Plus, X } from "lucide-react";
...
const steps = [
  ...
  ["Project Experience", "Tell us about projects you have worked on"],
...
function savedProjects(draft: Draft): ProjectDraft[] {
  try {
    const value: unknown = JSON.parse(String(draft.project_records ?? "[]"));
...
function SkillPicker({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  ...
  const selected = lines(value);
  function toggle(skill: string) {
    onChange(selected.some((item) => item.toLowerCase() === skill.toLowerCase())
      ? selected.filter((item) => item.toLowerCase() !== skill.toLowerCase()).join(", ")
      : [...selected, skill].join(", "));
  }
  function addCustomSkill() {
    const skill = customSkill.trim();
    if (skill && !selected.some((item) => item.toLowerCase() === skill.toLowerCase())) onChange([...selected, skill].join(", "));
    setCustomSkill("");
  }
  return (
    <section className="field" aria-label={label}>
      <span className={styles.skillLabel}>{label}</span>
      <div className={styles.skillPicker} role="group" aria-label={`${label} suggestions`}>
        {suggestedSkills.map((skill) => {
          const active = selected.some((item) => item.toLowerCase() === skill.toLowerCase());
          return <button key={skill} type="button" className={styles.skillChip} data-selected={active} aria-pressed={active} onClick={() => toggle(skill)}>{active ? <Check aria-hidden="true" /> : <Plus aria-hidden="true" />}{skill}</button>;
        })}
      </div>
      <div className={styles.customSkill}>
        <Input id={`${label.toLowerCase().replaceAll(" ", "-")}-custom-skill`} label="Add another skill" value={customSkill} onChange={(event) => setCustomSkill(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addCustomSkill(); } }} />
        <Button type="button" variant="quiet" disabled={!customSkill.trim()} onClick={addCustomSkill}><Plus aria-hidden="true" /> Add skill</Button>
      </div>
      {selected.length ? <div className={styles.selectedSkills} aria-label="Selected skills">{selected.map((skill) => <span key={skill}>{skill}<button type="button" aria-label={`Remove ${skill}`} onClick={() => toggle(skill)}><X aria-hidden="true" /></button></span>)}</div> : <p className={styles.skillHint}>Choose all that apply, or add a skill of your own.</p>}
    </section>
  );
}
...
export function StudentOnboardingWizard() {
  ...
  async function save(advance: boolean) {
    ...
    if (advance && step === 5 && draft.review_confirmed !== true) {
      setState("error");
      setMessage("Review and confirm your profile before entering the workspace.");
      return;
    }
  ...
  async function skipNonRequiredStep() {
    ...
    setMessage("");
    try {
      const saved = await sendStep(data, skippedStep + 1, {});
      setData(saved);
      ...
      if (cause instanceof ApiError && cause.status === 409) setState("conflict"); else setState("error");
      setMessage(cause instanceof ApiError ? cause.message : "Could not skip this step. Your tab draft is still available.");
    } finally { saving.current = false; }
  }
  ...
  function saveProject() {
    ...
    const projectUrl = String(draft.project_url).trim();
    let validUrl = true;
    if (projectUrl) {
      try { validUrl = ["http:", "https:"].includes(new URL(projectUrl).protocol); }
      catch { validUrl = false; }
    }
    if (title.length < 2 || description.length < 10 || description.length > 300 || !validUrl) {
      setState("error");
      setMessage("Add a project title, a description between 10 and 300 characters, and a valid web link.");
      return;
    }
    const project: ProjectDraft = {
      title,
      project_type: String(draft.project_type) as ProjectType,
      description,
      technologies: lines(draft.project_technologies),
      project_url: projectUrl,
    };
    setDraft((current) => ({ ...current, project_records: JSON.stringify([...savedProjects(current), project]), project_title: "", project_type: "academic", project_description: "", project_technologies: "", project_url: "" }));
    setDirty(true);
    setState("idle");
    setMessage("");
  ...
  function editProject(index: number) {
    const projects = savedProjects(draft);
    const [project] = projects.splice(index, 1);
    if (!project) return;
    setDraft((current) => ({ ...current, project_records: JSON.stringify(projects), project_title: project.title, project_type: project.project_type, project_description: project.description, project_technologies: project.technologies.join(", "), project_url: project.project_url }));
    setDirty(true);
    setAddingProject(true);
  }
  function removeProject(index: number) {
    setDraft((current) => ({ ...current, project_records: JSON.stringify(savedProjects(current).filter((_, itemIndex) => itemIndex !== index)) }));
    setDirty(true);
  }
        ...
        <section className={styles.panel}>
          <header className={styles.panelHeader}>
            <div>
              ...
              <span>{step === 5
                ? "Confirm your placement choices and review your profile before entering the workspace."
                : `${steps[step - 1][1]}.`}</span>
            </div>
            <div className={styles.saveStatus} data-state={state}>
              <Cloud aria-hidden="true" />
              {state === "saving" ? "Saving…" : state === "saved" ? "Saved" : state === "conflict" ? "Reload required" : "Autosave on"}
            </div>
          </header>
          ...
          <form className={styles.form} onSubmit={submit}>
            ...
            {step === 3 ? <>
              <fieldset className={styles.projectQuestion}>
                <legend>Have you worked on any projects?</legend>
                {(["yes", "no"] as const).map((answer) => <label key={answer}><input type="radio" name="has_projects" value={answer} checked={draft.has_projects === answer} onChange={() => change("has_projects", answer)} />{answer === "yes" ? "Yes" : "No"}</label>)}
              </fieldset>
              {draft.has_projects === "yes" ? <section className={styles.projectSection} aria-label="Project experience">
                {savedProjects(draft).map((project, index) => <article className={styles.projectCard} key={`${project.title}-${index}`}>
                  <div><span>{projectTypeLabels[project.project_type]}</span><h3>{project.title}</h3><p>{project.description}</p>{project.technologies.length ? <small>{project.technologies.join(" · ")}</small> : null}{project.project_url ? <a href={project.project_url} target="_blank" rel="noreferrer">Open project link</a> : null}</div>
                  <div className={styles.projectActions}><Button type="button" variant="quiet" onClick={() => editProject(index)}>Edit</Button><Button type="button" variant="quiet" onClick={() => removeProject(index)}>Remove</Button></div>
                </article>)}
                {addingProject ? <div className={styles.projectEditor}>
                  <h3>{savedProjects(draft).length ? "Add another project" : "Add Project"}</h3>
                  <Input id="project_title" label="Project Title" placeholder="e.g. Campus Placement Portal" maxLength={160} value={String(draft.project_title)} onChange={(e) => change("project_title", e.target.value)} />
                  <Select id="project_type" label="Project Type" value={String(draft.project_type)} onChange={(e) => change("project_type", e.target.value)}><option value="academic">Academic</option><option value="personal">Personal</option><option value="internship">Internship</option><option value="hackathon">Hackathon</option><option value="other">Other</option></Select>
                  <div className={`${styles.fieldRow} ${styles.projectDescription}`}>
                    <label htmlFor="project_description">What did you work on?</label>
                    <span id="project_description_hint">Briefly describe what you built or contributed.</span>
                    <textarea id="project_description" aria-describedby="project_description_hint project_description_count" maxLength={300} minLength={10} rows={4} value={String(draft.project_description)} onChange={(e) => change("project_description", e.target.value)} />
                    <small id="project_description_count">{String(draft.project_description).length}/300 characters</small>
                  </div>
                  <div className={styles.projectSkills}><SkillPicker label="Skills / Technologies Used" value={String(draft.project_technologies)} onChange={(value) => change("project_technologies", value)} /></div>
                  <Input id="project_url" label="Project link" placeholder="https://..." hint="Share a GitHub, live demo, or portfolio link if available." type="url" value={String(draft.project_url)} onChange={(e) => change("project_url", e.target.value)} />
                  <Button type="button" onClick={saveProject}>Save Project</Button>
                </div> : <Button type="button" variant="quiet" onClick={() => setAddingProject(true)}><Plus aria-hidden="true" /> Add Another Project</Button>}
              </section> : null}
              <div className={styles.projectSkills}><SkillPicker label="Profile skills" value={String(draft.skills)} onChange={(value) => change("skills", value)} /></div>
              <Input id="certification_name" label="Certification" value={String(draft.certification_name)} onChange={(e) => change("certification_name", e.target.value)} />
              <Input id="certification_issuer" label="Certification issuer" value={String(draft.certification_issuer)} onChange={(e) => change("certification_issuer", e.target.value)} />
            </> : null}
            {step === 4 ? <>
              ...
              <Input id="industries" label="Industries" value={String(draft.industries)} onChange={(e) => change("industries", e.target.value)} />
              <Input id="locations" label="Locations" value={String(draft.locations)} onChange={(e) => change("locations", e.target.value)} />
