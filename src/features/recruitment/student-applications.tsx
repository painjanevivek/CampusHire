  ...
  return (
    <main id="main-content" className={styles.page}>
      ...
      {loading ? <p role="status">Loading application history…</p> : !items.length ? (
        ...
        return <li key={item.id}><div><p>{role.company_name ?? "Published opportunity"}</p><h2>{role.title ?? "Placement application"}</h2><span>Submitted {formatDate(item.created_at)}</span><details><summary>View status history</summary><ol>{item.history.map((event) => <li key={event.id}><strong>{event.to_status.replaceAll("_", " ")}</strong><span>{formatDate(event.created_at)}{event.reason ? ` · ${event.reason}` : ""}</span></li>)}</ol></details></div><div><strong data-status={item.status}>{item.status.replaceAll("_", " ")}</strong><small>Resume v{String(item.resume_snapshot.version_number ?? "—")} · Rule v{String(item.rule_snapshot.version ?? "—")}</small></div></li>;
      })}</ol>}
    </main>
  );
}
