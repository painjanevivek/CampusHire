...
export function StudentApplicationDetail({ applicationId }: { applicationId: string }) {
  ...
  if (!application) return <main id="main-content" className={styles.page}><h1>Application record</h1>{error ? <Alert tone="error">{error} <button type="button" onClick={() => void load()}>Retry</button></Alert> : <p role="status">Loading immutable decision evidence…</p>}</main>;
  ...
  return <main id="main-content" className={styles.page}>
    ...
    <header><div><p>{role.company_name ?? "Published opportunity"}</p><h1>{role.title ?? "Application record"}</h1><span>Submitted {formatDate(application.created_at)} · Institution-local display</span></div><strong>{application.status.replaceAll("_", " ")}</strong></header>
    {error ? <Alert tone="warning">{error}</Alert> : null}
    <section className={styles.summary} aria-label="Locked application evidence">
      ...
      <article><Scale aria-hidden="true" /><p>Decision version</p><h2>Rule v{String(application.rule_snapshot.version ?? "—")}</h2><span>{String(application.decision_snapshot.eligibility_fingerprint ?? "").slice(0, 12)} · eligibility evidence</span></article>
      <article><CalendarPlus aria-hidden="true" /><p>Application deadline</p><h2>{role.deadline_at ? formatDate(role.deadline_at) : "Not available"}</h2>{role.deadline_at ? <a href={apiPath(`/applications/${application.id}/deadline.ics`)} download>Download calendar file</a> : null}</article>
    </section>
    <section className={styles.history} aria-labelledby="history-title"><header><p>Status history</p><h2 id="history-title">A complete, append-only timeline</h2></header><ol>{application.history.map((event) => <li key={event.id}><CheckCircle2 aria-hidden="true" /><div><strong>{event.to_status.replaceAll("_", " ")}</strong><time dateTime={event.created_at}>{formatDate(event.created_at)}</time>{event.reason ? <p>{event.reason}</p> : null}</div></li>)}</ol></section>
    ...
    <section className={styles.actions} aria-label="Application actions">
      ...
      {!openAppeal && application.status !== "withdrawn" ? <details><summary>Request an appeal or manual review</summary><form onSubmit={appeal}><label>Request type<select name="kind"><option value="manual_review">Manual evidence review</option><option value="appeal">Decision appeal</option></select></label><label>Reason<textarea name="reason" required minLength={20} maxLength={1000} /></label><label>Supporting evidence labels (optional)<input name="evidence" placeholder="Education record, reviewed resume" /></label><label className={styles.confirm}><input name="confirmation" type="checkbox" required /> I confirm this request is accurate and ready for review.</label><button disabled={busy} type="submit">Submit review request</button></form></details> : null}
    </section>
  </main>;
}
