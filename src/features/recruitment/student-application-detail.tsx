...
export function StudentApplicationDetail({ applicationId }: { applicationId: string }) {
  ...
  async function appeal(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      formElement.reset();
      setNotice("Your review request was submitted with the supporting details you provided.");
    } catch (caught) {
      if (outcomeIsUnknown(caught)) {
        setError(
  ...
  return <main id="main-content" className={styles.page}>
    ...
    {application.appeals.length ? <section className={styles.appeals} aria-labelledby="appeals-title"><h2 id="appeals-title">Review requests</h2>{application.appeals.map((item) => <article key={item.id}><strong>{item.kind.replaceAll("_", " ")} · {item.status.replaceAll("_", " ")}</strong><p>{item.reason}</p><dl className={styles.appealMeta}><div><dt>Responsible team</dt><dd>{appealOwnerLabel(item)}</dd></div><div><dt>Target response</dt><dd>{item.due_at ? formatDate(item.due_at, application.institution_timezone) : "Not scheduled"}</dd></div><div><dt>Due state</dt><dd>{appealEscalationLabel(item.escalation_state)}</dd></div><div><dt>Independence</dt><dd>{item.independence_status?.replaceAll("_", " ") ?? "Awaiting assignment"}</dd></div></dl>{item.supporting_evidence.length ? <><h3>Supporting details</h3><ul>{item.supporting_evidence.map((evidence) => <li key={evidence}>{evidence}</li>)}</ul></> : null}{item.administrator_response ? <blockquote>{item.administrator_response}{item.resolution_effect ? <footer>Effect: {item.resolution_effect.replaceAll("_", " ")}</footer> : null}</blockquote> : <span>Awaiting the placement review team. Platform administrators can arrange staffing but do not decide appeal merits.</span>}</article>)}</section> : null}
