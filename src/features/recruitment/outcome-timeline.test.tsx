...
describe("OutcomeTimeline", () => {
  ...
  it("keeps an offer distinct from joining", async () => {
    ...
    expect(screen.getByText(/an offer does not confirm that you joined/i)).toBeInTheDocument();
    expect(screen.queryByText("Joined")).not.toBeInTheDocument();
    expect(screen.getByText("Supporting record on file", { exact: false })).toBeInTheDocument();
  });
  ...
  it("lets an officer record a separate verified joining event", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Outcome status"), { target: { value: "verified" } });
    ...
    fireEvent.change(screen.getByLabelText("Supporting record link"), { target: { value: "private://joining-confirmation" } });
    ...
    expect(await screen.findByText(/Records already on file were not changed/i)).toBeInTheDocument();
  });
});
