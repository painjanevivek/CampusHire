...
export function AdminApplications() {
  ...
  function fail(cause: unknown) {
    setError(cause instanceof ApiError && cause.status === 409
      ? "This record or preview changed. Refresh and review the latest student details before trying again. No silent override was made."
  ...
  return <main id="main-content" className={styles.page} data-navigation-ready={!loading && !detailLoading && (!detailId || !!selected) && !error}>
    <header className={styles.header}><div><p>Accountable review</p><h1>Applications</h1><span>Review student records, request missing details, and record a reasoned decision.</span></div>
    ...
    <div className={styles.workspace} data-detail-open={!!selectedId}>
      ...
      {!!selectedId && <section data-selected-application={!detailLoading ? selected?.id : undefined} className={styles.inspector} aria-label="Candidate decision details" aria-busy={detailLoading}>
        ...
        {detailLoading && <p role="status">Loading student profile and application details…</p>}
        {selected ? <div className={ui.stack}>
          <header><h2 tabIndex={-1} ref={headingRef}>{selected.student_name}</h2><p>{String(selected.role_snapshot.title)} · {String(selected.role_snapshot.company_name)}</p>
            <Badge tone="neutral">{selected.status.replaceAll("_", " ")}</Badge><div className={styles.responsibilityStrip} aria-label="Case responsibility"><div><span>Owner</span><strong>{selected.assignee_user_id ? "Assigned reviewer" : "Unassigned queue"}</strong></div><div><span>Due</span><strong>{selected.review_due_at ? new Date(selected.review_due_at).toLocaleString(undefined, { timeZone: selected.institution_timezone }) : "Not scheduled"}</strong></div><div><span>Next action</span><strong>{selectedRow ? nextAction(selectedRow) : selected.next_step ?? "Review application details"}</strong></div><Badge tone={selected.due_state === "overdue" ? "warning" : "neutral"}>{selected.due_state?.replaceAll("_", " ") ?? "Not assigned"}</Badge></div><p className={styles.guidance}>{selectedRow?.awaiting_review ? "Student response received. Review the new information." : selectedRow?.open_requests ? "Awaiting the student's response to an information request." : "Review the application and the documents the student provided."}</p><p className={styles.updated}>Last change {new Date(selected.updated_at).toLocaleString(undefined, { timeZone: selected.institution_timezone })} ({selected.institution_timezone})</p>{canBulk && !selected.assignee_user_id ? <button className={ui.primary} type="button" disabled={busy} onClick={() => void claim()}>Claim application</button> : !!selected.allowed_actions?.length ? <a className={ui.primary} href="#review-decision">Go to review decision</a> : null}</header>
          <section className={styles.evidence}><h3>Eligibility result</h3><p>{selected.eligibility_snapshot.status.replaceAll("_", " ")}</p>
            ...
            <details><summary>Rule and decision details</summary><pre className={ui.tableWrap}>{JSON.stringify({ rules: selected.rule_snapshot, decision: selected.decision_snapshot }, null, 2)}</pre></details>
          </section>
          <section className={styles.evidence}><h3>Submitted resume</h3><p>{String(selected.resume_snapshot.original_name ?? "Reviewed resume")} · version {String(selected.resume_snapshot.version_number ?? "Not recorded")}</p>
            <details><summary>Saved resume and profile details</summary><pre className={ui.tableWrap}>{JSON.stringify({ resume: selected.resume_snapshot, profile: selected.profile_snapshot }, null, 2)}</pre></details></section>
          ...
          <details className={styles.evidence}><summary>Decision history</summary><ol className={ui.timeline}>{selected.history.map(item => <li key={item.id}><p>{item.to_status.replaceAll("_", " ")} · {new Date(item.created_at).toLocaleString()}</p><p>{item.reason ?? "No additional reason recorded"}</p><details><summary>Action recorded by</summary><code>{item.actor_user_id}</code></details></li>)}</ol></details>
        ...
        </div> : !detailLoading && <p>Select an application to review its details.</p>}
      </section>}
    </div>
  </main>;
}
