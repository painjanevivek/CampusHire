...
export function CommunicationPreferences() {
  ...
  return <section className={styles.panel} aria-labelledby="communication-title" aria-busy={!preferences}>
    ...
    <div className={styles.options}>
      ...
      <label><span><strong>Deadline reminders</strong><small>Optional reminders may be suppressed when delivery capacity is constrained.</small></span><input type="checkbox" checked={preferences?.deadline_reminders ?? false} disabled={!preferences} onChange={(event) => void change("deadline_reminders", event.target.checked)} /></label>
