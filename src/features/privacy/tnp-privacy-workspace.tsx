...
export function TnpPrivacyWorkspace() {
    ...
    try {
      await csrfRequest(`/tnp/privacy/requests/${item.id}`, {
        ...
        body: JSON.stringify({
          ...
          resolution_effect: ["approve", "decline", "complete"].includes(action) ? data.get("resolution_effect") : null,
  ...
  return <PageContainer context="admin" className={styles.page}>
    ...
    <section className={styles.list} aria-label="Institution privacy requests">
      {requests.data?.map(item => <article className={styles.request} key={item.id}><header><strong>{item.request_type.replaceAll("_", " ")}</strong><p>{item.details || "No additional details submitted."}</p><small>Submitted {new Date(item.created_at).toLocaleString()}</small></header><Badge tone={item.status === "completed" ? "success" : item.status === "held" || item.status === "failed" ? "warning" : "neutral"}>{item.status.replaceAll("_", " ")}</Badge><details open={!item.owner_user_id && !item.completed_at}><summary>Ownership, timing, and decision</summary><p>Owner: {item.owner_user_id ? "Assigned institution officer" : "Unassigned queue"} · Due: {item.due_at ? new Date(item.due_at).toLocaleString() : "Not scheduled"}</p>{item.result_summary ? <p>Result: {item.result_summary}</p> : null}{item.receipt_reference ? <p>Receipt: <code>{item.receipt_reference}</code></p> : null}{item.resolution_effect ? <p>Effect: {item.resolution_effect.replaceAll("_", " ")}</p> : null}<dl>{Object.entries(item.processing_receipt ?? {}).map(([key, value]) => <div key={key}><dt>{key.replaceAll("_", " ")}</dt><dd>{String(value).replaceAll("_", " ")}</dd></div>)}</dl>{item.user_id && !holds.data?.some(hold => hold.user_id === item.user_id && !hold.released_at) ? <details><summary>Create a retention hold</summary><form className={styles.form} onSubmit={(event) => void createHold(event, item)}><label>Review date<input name="review_at" type="datetime-local" required /></label><label>Legal or policy reason<input name="reason" minLength={10} maxLength={1000} required /></label><Button disabled={busy === `hold:${item.id}`}>Record hold</Button></form></details> : null}{!item.completed_at && item.status !== "declined" ? <form className={styles.form} onSubmit={(event) => void decide(event, item)}><label>Action<select name="action" defaultValue={item.owner_user_id ? "approve" : "assign"}><option value="assign">Assign to me</option><option value="approve">Approve / begin processing</option><option value="hold">Record valid hold</option><option value="decline">Decline with reason</option><option value="complete">Complete non-erasure request</option></select></label><label>Resolution effect<select name="resolution_effect" defaultValue="request_fulfilled"><option value="request_fulfilled">Request fulfilled</option><option value="eligible_data_erased">Eligible data erased</option><option value="record_corrected">Record corrected</option><option value="consent_withdrawn">Optional consent withdrawn</option><option value="grievance_resolved">Grievance resolved</option><option value="request_declined">Request declined</option></select></label><label>Reason or outcome<input name="reason" minLength={10} maxLength={2000} required placeholder="Explain this accountable action" /></label><Button disabled={busy === item.id}>{busy === item.id ? "Saving…" : "Confirm action"}</Button></form> : null}</details></article>)}
